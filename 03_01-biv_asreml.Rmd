...
Here the code to extract the BLUPs.
...
```{r}
plot(BLUP.tarsus ~ BLUP.btw, BLUPS, xlab = "", ylab = "", xlim=c(-4,4), ylim=c(-7,7),las = 1.2, bty = "o", col = "white")
...
```
Here, the LRT test shows that the both models are significantly different. Meaning the correlation is different than 0.


In addition, it is also possible to test if sexes has significant differences with another reduced model where both covariance are fixed to their average values. The codes are provide as example, however the model can not run on this dataset.
...
mtext("Covariances (±CI)", side = 1, las = 1, adj = 0.4, line = 3, cex = 1.6)
...
par(mfrow = c(1, 2))
#
plot(BLUP.tarsus ~ BLUP.btw, FEM, xlab = "", ylab = "", las = 1.2, bty = "o", col = "white", xlim=c(-3,3),ylim=c(-9,9))
...
mtext("tarsus (BV±SE)", side = 2, line = 2, las = 3)
#
plot(BLUP.tarsus ~ BLUP.btw, MAL, xlab = "", ylab = "", las = 1.2, bty = "o", col = "white", xlim=c(-3,3),ylim=c(-9,9))
...
plot(X, Y, xlab = "", ylab = "", las = 1.2, bty = "o", col = "white",xlim=(-7,-7),ylim=(-5,5))
