...
<!-- C2-5 you don't compare with and without NA -->
...
<!--Issue with corgh structure at the residual level, no idea why since the structure us works perfectly  -->
...
```{r}
DvsS<-data.frame(Trait = colnames(model2.5$Sol),
                 BLUP = posterior.mode(model2.5$Sol),
                 CI = HPDinterval((model2.5$Sol)))
DvsS<-DvsS[5:5240,] #keep only rows associated with animal
...
```{r}
DvsS<-data.frame(Trait = colnames(model.BivSex$Sol),
                 BLUP = posterior.mode(model.BivSex$Sol),
                 CI = HPDinterval((model.BivSex$Sol)))
DvsS<-DvsS[3:2619,] #keep only rows associated with animal
