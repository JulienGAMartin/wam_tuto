...
(h2.3 <- vpredict(model4, h2.bwt ~ V1 / (V1 + V2+ V3)))
...
First, the dataset need to be order by group (sex here). The variance need to be partitionned or fragmented at the random and residual levels. At the radnom effect, the fonction `at()` is perfect for this example. For the residual level, it is required to used the fonction `dsum()` 
...
Here, we can see the $h^2$ seems to differ between sexe (`r round(h2.F[1],2)` and `r round(h2.M[1],2)`), the LRT partitionning the variance and the residual between group (sex) did not improved the fit of the model and so their variance are not significantly different. 
...
temp2 <- asreml(
  fixed = bwt ~ 1,
  random=~str(~vm(animal,ainv)+mother,~us(2):id(xx)) + byear,
  residual = ~ idv(units),
  data = gryphon,
  na.action = na.method(x = "omit", y = "omit"),
  start.values=T)
...
length(unique(c(gryphon$animal, gryphon$mother)))#1084
