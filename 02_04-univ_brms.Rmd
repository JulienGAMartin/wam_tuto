...
Unfortunately brms can not partition the residual variance ('sigma level`) into different level yet.So the followed model partitioned only the additive genetic variance. 

 <!-- Here maybe an issue  if i try to partion the random effect byear or mother I got an error message "  "Some levels of 'mother' correspond to multiple levels of 'sex'.  I dont understand why ?? 
 so I kept the model simple with just animal bwing partitioningby sex-->
...
#  bwt ~ 1 +  sex + (1 | gr(animal, cov = Amat, by = sex))+ (1 | gr(byear, by = sex)) + (1 | gr(mother, by = sex)),
  bwt ~ 1 +  sex + (1 | gr(animal, cov = Amat, by = sex))+ (1 | byear) + (1 | mother),  
  data = gryphon,
  data2 = list(Amat = Amat),
  family = gaussian(),
  chains = 2, cores = 2, iter = 1000
