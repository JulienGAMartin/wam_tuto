...
Here then the repeatability of the trait `laydate` can be determined by hand as `r round(intercept_var = attr(VarCorr(modelv)$animal,'stddev')[1]^2/(intercept_var = attr(VarCorr(modelv)$animal,'stddev')[1]^2+attr(VarCorr(modelv),'sc')^2` (_i.e._, as `r round(intercept_var = attr(VarCorr(modelv)$animal,'stddev')[1]^2)`/(`r round(intercept_var = attr(VarCorr(modelv)$animal,'stddev')[1]^2)` + `r round(attr(VarCorr(modelv)$animal,'stddev')[1]^2)`)).
...
The repeatability of lay date, after accounting for age effects, is now estimated as `r round(intercept_var = attr(VarCorr(modelw)$animal,'stddev')[1]^2/(intercept_var = attr(VarCorr(modelw)$animal,'stddev')[1]^2+attr(VarCorr(modelw),'sc')^2` (_i.e._, as `r round(intercept_var = attr(VarCorr(modelw)$animal,'stddev')[1]^2)`/(`r round(intercept_var = attr(VarCorr(modelw)$animal,'stddev')[1]^2)` + `r round(attr(VarCorr(modelw)$animal,'stddev')[1]^2)`)).
So, just as we saw when estimating $h^2$ in Tutorial 1, the inclusion of fixed effects will alter the estimated effect size <!-- effect size or just the variance estimate --> if we determine total phenotypic variance as the sum of the variance components. Thus, proper interpretation is vital.
