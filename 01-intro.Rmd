...
`gremlin` is a little monster appearing <!-- water just multiply them (if you wet a mugwai and--> feed it after midnight. It is also a great and promising software to fit mixed models using a frequentist approach.
...
<!-- i think we have to also cite Pierre de Villemereuil, he wrote a guide for estimating heritability with MCMCglmmm http://devillemereuil.legtux.org/wp-content/uploads/2012/12/tuto_en.pdf -->
...
It should be noted that if `brms` is able to fit animal model the parametization/setting used based on Kroeneker products is not the most efficient and can take quite longer than using a DAG parametrisation directly in `stan`. <!-- what is a DAG?-->
